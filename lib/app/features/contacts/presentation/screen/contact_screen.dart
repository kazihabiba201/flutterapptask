...
class _ContactScreenState extends State<ContactScreen> {
  ...
  void _showAlertForm(int? id) async{
    if(id !=null){
      final existingContacts = _contacts.firstWhere((element)=> element['id'] == id);
  ...
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: Pallets.surfaceColor,
            ...
            Expanded(
              child: Column( mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  ...
                  if (_contacts.isEmpty)  Center(child: Text(translation(context).noContactsFound +  '\n'+translation(context).pleaseAddANewContact, style: TextStyle(fontSize: 15, ), textAlign: TextAlign.center,)) else Expanded(
                    child: ListView.builder(itemCount: _contacts.length, itemBuilder: (context, index)=> Card(
                      color: Pallets.colorBlue, margin: const EdgeInsets.all(8), child: Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          Column(crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              SizedBox(width: 250, child: Text('Name:  ${_contacts[index]['name']}', style: const TextStyle(fontSize: 20, color: Colors.white),overflow: TextOverflow.ellipsis,)),
                              const Gap(5),
                              Text('Number:  ${_contacts[index]['phone']}', style: const TextStyle(fontSize: 20, color: Colors.white),),
                            ],
                          ),
                          InkWell(onTap: (){_deletItem(_contacts[index]['id']);}, child: const Icon(Icons.cancel, color: Colors.white, size: 20,))
                        ],
                      ),
                    ),
                    )),
